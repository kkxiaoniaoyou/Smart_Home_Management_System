...
char * ESP8266_ReceiveString ( FunctionalState enumEnUnvarnishTx )
{
	...
	// 配置 MQTT 用户属性
bool ESP8266_MQTT_UserConfig(char *Clientid,char *Username,char *Password)
{
	char cCmd [120];
	if(Clientid!=NULL)
		sprintf ( cCmd, "AT+MQTTUSERCFG=0,1,\"%s\",\"%s\",\"%s\",0,0,\"\"", Clientid, Username, Password );
	else
		sprintf ( cCmd, "AT+MQTTUSERCFG=0,1,\"NULL\",\"%s\",\"%s\",0,0,\"\"", Username, Password );
	
	return ESP8266_Cmd ( cCmd, "OK", 0, 1000 );
}

//设置ClientID
bool ESP8266_MQTT_ClientIdSet(char *ClientId)
{
	char cCmd [120];
	sprintf (cCmd,"AT+MQTTCLIENTID=0,\"%s\"",ClientId);
	return ESP8266_Cmd(cCmd,"OK",0,2000);
}

//连接服务器
bool ESP8266_MQTT_Connect(char *HostURL,char *port)
{
	char cCmd [120];
	sprintf (cCmd,"AT+MQTTCONN=0,\"%s\",%s,1",HostURL,port);
	return ESP8266_Cmd(cCmd,"OK",0,6000);
}
...
//获取时间,调用NTP服务器的时间
void ESP8266_Time(u8 *p)
{
	u8 *p1;
	u8 *p2;
	strEsp8266_Fram_Record .InfBit .FramLength = 0;               //从新开始接收新的数据包
	ESP8266_Usart ("AT+CIPSNTPTIME?\r\n");
	delay_ms ( 200 );                 //延时
	strEsp8266_Fram_Record .Data_RX_BUF [ strEsp8266_Fram_Record .InfBit .FramLength ]  = '\0';
	PC_Usart ( "%s", strEsp8266_Fram_Record .Data_RX_BUF );
